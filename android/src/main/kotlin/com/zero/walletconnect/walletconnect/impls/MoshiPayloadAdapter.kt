...
import com.zero.walletconnect.walletconnect.types.*
...
class MoshiPayloadAdapter(moshi: Moshi) : Session.PayloadAdapter {
                ...
                mapAdapter.fromJson(json)?.let {
                    try {
                        ...
                        when (method) {
                            ...
                            "personal_sign" -> it.toPersonalSign()
                            "eth_sign" -> it.toSign()
                            "eth_signTypedData" -> it.toSignTypedData()
                            "eth_sendTransaction" -> it.toSendTransaction()
                            "eth_signTransaction" -> it.toSignTransaction()
                            "eth_sendRawTransaction" -> it.toSendRawTransaction()
                            null -> it.toResponse()
                            else -> it.toCustom()
                        }
                    } catch (e: Exception) {
    ...
    private fun Map<String, *>.toSendTransaction(): Session.MethodCall.ETHSendTransaction {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        ...
        val gasLimit = data["gas"] as? String
        val value = data["value"] as? String ?: "0x0"
        val txData = data["data"] as? String ?: throw IllegalArgumentException("data key missing")
        return Session.MethodCall.ETHSendTransaction(getId(), from, to, nonce, gasPrice, gasLimit, value, txData)
    }

    private fun Map<String, *>.toSignTransaction(): Session.MethodCall.ETHSignTransaction {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        val data = params.firstOrNull() as? Map<*, *>
                ?: throw IllegalArgumentException("Invalid params")
        val from = data["from"] as? String ?: throw IllegalArgumentException("from key missing")
        ...
        val value = data["value"] as? String ?: "0x0"
        val txData = data["data"] as? String ?: throw IllegalArgumentException("data key missing")
        return Session.MethodCall.ETHSignTransaction(getId(), from, to, nonce, gasPrice, gasLimit, value, txData)
    }

    private fun Map<String, *>.toSendRawTransaction(): Session.MethodCall.ETHSendRawTransaction {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        val address = params.getOrNull(0) as? String
                ?: throw IllegalArgumentException("Missing address")
        return Session.MethodCall.ETHSendRawTransaction(getId(), address)
    }

    private fun Map<String, *>.toSign(): Session.MethodCall.ETHSign {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        val address = params.getOrNull(0) as? String
                ?: throw IllegalArgumentException("Missing address")
        val message = params.getOrNull(1) as? String
                ?: throw IllegalArgumentException("Missing message")
        return Session.MethodCall.ETHSign(getId(), address, message)
    }

    private fun Map<String, *>.toSignTypedData(): Session.MethodCall.ETHSignTypedData {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        val account = params.getOrNull(0) as? String
                ?: throw IllegalArgumentException("Missing address")
        val message = params.getOrNull(1) as? String
                ?: throw IllegalArgumentException("Missing message")
        return Session.MethodCall.ETHSignTypedData(getId(), account, message)
    }

    private fun Map<String, *>.toPersonalSign(): Session.MethodCall.PersonalSign {
        val params = this["params"] as? List<*> ?: throw IllegalArgumentException("params missing")
        val message = params.getOrNull(0) as? String
                ?: throw IllegalArgumentException("Missing message")
        val account = params.getOrNull(1) as? String
                ?: throw IllegalArgumentException("Missing address")
        return Session.MethodCall.PersonalSign(getId(), message, account)
    }
    ...
    private fun Session.MethodCall.toBytes() =
            mapAdapter.toJson(
                    when (this) {
                        ...
                        is Session.MethodCall.ETHSendTransaction -> this.toMap()
                        is Session.MethodCall.ETHSign -> this.toMap()
                        is Session.MethodCall.Custom -> this.toMap()
                        is Session.MethodCall.PersonalSign -> this.toMap()
                        else -> (this as Session.MethodCall.Custom).toMap()//todo
                    }
            ).toByteArray()
    ...
    private fun Session.MethodCall.ETHSendTransaction.toMap() =
            jsonRpc(
    ...
    private fun Session.MethodCall.ETHSign.toMap() =
            jsonRpc(
                    id, "eth_sign", address, message
            )

    private fun Session.MethodCall.PersonalSign.toMap() =
            jsonRpc(
                    id, "personal_sign", message, account
            )
