...
import com.zero.walletconnect.walletconnect.nullOnThrow
import com.zero.walletconnect.walletconnect.types.extractSessionParams
import com.zero.walletconnect.walletconnect.types.intoMap
...
class WCSession(
    ...
    private fun handleMessage(message: Session.Transport.Message) {
        ...
        when (data) {
            ...
            is Session.MethodCall.PersonalSign -> {
                accountToCheck = data.account
            }
            is Session.MethodCall.ETHSign -> {
                accountToCheck = data.address
            }
            is Session.MethodCall.ETHSignTypedData->{
                accountToCheck = data.account
            }
            is Session.MethodCall.ETHSendTransaction -> {
                accountToCheck = data.from
            }
            is Session.MethodCall.ETHSignTransaction->{
                accountToCheck = data.from
            }
